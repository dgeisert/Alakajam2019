...
public class ProceduralSplat : MonoBehaviour
{
    ...
    void Branch(Quaternion quaternion, Vector3 position, int lastRingVertexIndex, float radius, float texCoordV, int step = 0)
    {
        ...
        if (radius < MinimumRadius || step >= maxSteps) // End branch if reached minimum radius, or ran out of vertices
        {
            ...
            else
            {
                ...
                for (var n = island.vertsSplats.Count - NumberOfSides - 2; n < island.vertsSplats.Count - 2; n++)
                { // Add cap
                    island.trianglesSplats.Add(n);
                    island.trianglesSplats.Add(island.vertsSplats.Count - 1);
                    island.trianglesSplats.Add(n + 1);
    ...
    void AddLeaves(Quaternion quaternion, Vector3 position, int lastRingVertexIndex, float radius, int leafIndex)
    {
        ...
        for (var n = 0; n <= NumberOfSides; n++, ang += angInc)
        {
            ...
            offset.x = r * Mathf.Cos(ang); // Get X, Z vertex offsets
            offset.z = r * Mathf.Sin(ang);
            island.vertsSplatsB.Add(positionFactor + scalingFactor * (position + quaternion * offset)); // Add Vertex position
            island.uvsSplatsB.Add(new Vector2(Random.value * 10, Random.value * 10));
            island.colorListSplatsB.Add(leaves);
            texCoord.x += textureStepU;
        }
        ...
        if (leafIndex > 0)
        {
            for (var currentRingVertexIndex = island.vertsSplatsB.Count - NumberOfSides - 1; currentRingVertexIndex < island.vertsSplatsB.Count - 1; currentRingVertexIndex++, lastRingVertexIndex++)
            {
                island.trianglesSplatsB.Add(lastRingVertexIndex + 1); // Triangle A
                island.trianglesSplatsB.Add(lastRingVertexIndex);
                island.trianglesSplatsB.Add(currentRingVertexIndex);
                island.trianglesSplatsB.Add(currentRingVertexIndex); // Triangle B
                island.trianglesSplatsB.Add(currentRingVertexIndex + 1);
                island.trianglesSplatsB.Add(lastRingVertexIndex + 1);
            }
        }
        if (leafIndex >= leafArray.Length - 1)
        {
            ...
            if (pointEnd)
            {
                island.vertsSplatsB.Add(positionFactor + scalingFactor * (position + Vector3.up * SegmentLength)); // Add Vertex position
                island.uvsSplatsB.Add(new Vector2(Random.value * 10, Random.value * 10));
                island.colorListSplatsB.Add(leaves);
            }
            else
            {
                island.vertsSplatsB.Add(positionFactor + scalingFactor * (position)); // Add Vertex position
                island.uvsSplatsB.Add(new Vector2(Random.value * 10, Random.value * 10));
                island.colorListSplatsB.Add(leaves);
            }
            for (var n = island.vertsSplatsB.Count - NumberOfSides - 2; n < island.vertsSplatsB.Count - 2; n++) // Add cap
            {
                island.trianglesSplatsB.Add(n);
                island.trianglesSplatsB.Add(island.vertsSplatsB.Count - 1);
                island.trianglesSplatsB.Add(n + 1);
            }
            return;
        }
        ...
        lastRingVertexIndex = island.vertsSplatsB.Count - NumberOfSides - 1;
